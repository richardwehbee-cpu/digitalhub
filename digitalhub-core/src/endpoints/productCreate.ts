...
export class ProductCreate extends OpenAPIRoute {
	...
	async handle(c: AppContext) {
		...
		const requiredFields = ["name", "category", "product_group", "price", "stock"];
		const missing = requiredFields.filter(f => raw[f] === undefined || raw[f] === null || raw[f] === "");
		if (missing.length) {
			return c.json({
				success: false,
		...
		product.description = raw.description !== undefined && raw.description !== null ? String(raw.description).trim() : "";
		product.category = String(raw.category).trim();
		product.product_group = String(raw.product_group).trim();
		product.image = raw.image !== undefined && raw.image !== null ? String(raw.image) : "";
		product.region = raw.region !== undefined && raw.region !== null ? String(raw.region) : "";
		product.delivery_type = raw.delivery_type !== undefined && raw.delivery_type !== null ? String(raw.delivery_type) : "";
		...
		if (isNaN(stockNum) || !Number.isInteger(stockNum) || stockNum < 0) {
			return c.json({ success: false, message: "Stock must be a non-negative integer" }, 400);
		}
		product.stock = stockNum;

		// Optional numeric fields
		if (raw.cost_price !== undefined && raw.cost_price !== null) {
		...
		if (raw.compare_price !== undefined && raw.compare_price !== null) {
		...
		product.profit_percent = raw.profit_percent !== undefined && !isNaN(Number(raw.profit_percent)) ? Number(raw.profit_percent) : 30;
		product.discount_percent = raw.discount_percent !== undefined && !isNaN(Number(raw.discount_percent)) ? Number(raw.discount_percent) : 0;

		if (raw.active !== undefined && raw.active !== null) {
			const activeNum = Number(raw.active);
			product.active = (activeNum === 0 || activeNum === 1) ? activeNum : 1;
		} else {
			product.active = 1;
		}
		...
		if (!product.category) {
			return c.json({ success: false, message: "Category cannot be empty" }, 400);
		}
		if (!product.product_group) {
			return c.json({ success: false, message: "Product group cannot be empty" }, 400);
		}
		...
		const inventoryInsertStmt = c.env.DB
			.prepare(
				`INSERT INTO inventory (id, product_id, quantity, min_quantity, warehouse, updated_at)
				 VALUES (?, ?, ?, ?, ?, ?)`
			)
			.bind(
				inventoryId,
